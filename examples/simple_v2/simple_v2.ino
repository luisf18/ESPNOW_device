...
void espnow_handler( espnow_device_event_t evt, ESPNOW_connection *cn ){
  switch( evt ){
    ...
    case ESPNOW_DEVICE__EVT_NOTIFY: Serial.println("[H-NOTIFY]"); break;
...
void setup() {
  ...
  #else
    ESPNOW_device.set_handler( espnow_handler );
    ESPNOW_device.begin_client("RADIO_TTGO",1);
    ESPNOW_device.add_server( "ROBOT","1234" );
    //ESPNOW_device.add_server( "ESPNOW_Server2", "4321" );
    ESPNOW_device.frame.radio.init( 0 );
  #endif

  ESPNOW_device.log_level = 3;

  Serial.print( "send_delay: " ); Serial.println( ESPNOW_device.send_delay );
  Serial.print( "disconnect_delay: " ); Serial.println( ESPNOW_device.disconnect_delay );
}
